...
async def test_forbidden_marks_target_dead_then_short_circuits(isolate):
    ...
    metadata = {"delivery_event_kind": "final", "delivery_event_channel": "user"}
    res1 = await router.deliver("hi", [target], metadata=metadata)
    ...
    res2 = await router.deliver("hi again", [target], metadata=metadata)
